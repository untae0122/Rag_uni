...
results_path = args.trajs_path
# Remove existing file if it exists
if results_path and os.path.exists(results_path):
    os.remove(results_path)
    logger.info(f"Removed existing file: {results_path}")
...
def main():
    ...
    out_path: str = f'{args.output_dir}/preds_{args.shard_id}.jsonl'
    ...
    results: List[Dict] = []
    futures = {executor.submit(_generate_single_example, ex): ex for ex in data}
    
    with tqdm(total=len(data), desc="Processing examples") as pbar:
    ...
    results = [results_dict.get(ex.get('id')) for ex in data if results_dict.get(ex.get('id'))]
    ...
    with open(summary_path, 'w') as f:
        json.dump({
            'summary': {
                ...
                'accuracy_em_ratio': f"{accuracy_em_count}/{total}",
