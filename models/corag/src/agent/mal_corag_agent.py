...
class MalCoRagAgent:
    ...
    def __init__(
            self, vllm_client: VllmClient, corpus: Dataset, retriever=None, attack_manager: AttackManager = None, retrieval_mode=False
    ):
        ...
        if not self.attack_manager:
            raise ValueError("MalCoRagAgent requires an initialized AttackManager instance.")
    ...
    def sample_path(
        ...
        while len(past_subqueries) < max_path_length and num_llm_calls < max_num_llm_calls:
            ...
            # Retry handled inside AttackManager if using robust methods, or we loop here.
            # AttackManager's generate_adversarial_content handles simple parsing retries but maybe not full regeneration logic if I stripped it.
            # My AttackManager implementation has simple parsing logic. 
            # I should assume it returns valid (subanswer, corpuses) or (None, []).
            
            context_history = []
            for sq, sa in zip(past_subqueries, past_subanswers):
                context_history.append({'subquery': sq, 'subanswer': sa})

            adv_subanswer, corpuses = self.attack_manager.generate_adversarial_content(
                query=query,
            ...
            if not corpuses:
                print(f"[Warning] Attack generation failed for subquery: {subquery}")
                adv_subanswer = target_answer # Fallback?
                corpuses = []
            ...
            # Use AttackManager methods for injection
            if self.retrieval_mode: # Dynamic / Soft Oracle
                # poison_retriever adds to index and returns poisoned docs for reference
                self.attack_manager.poison_retriever(self.retriever, corpuses, subquery, qid, adv_subanswer or target_answer)
                
                # Now search (it will pick up poisoned docs if successful)
                subanswer, doc_ids, documents, poisoned_flags, retriever_results = self._get_subanswer_and_doc_ids(
                    subquery=subquery,
                    max_message_length=max_message_length
                )
            else: # Hard Oracle
                # Get oracle context directly
                retriever_results = self.attack_manager.get_oracle_context(corpuses, subquery, qid, adv_subanswer or target_answer)
                
                # We need to adapt these results to 'documents', 'doc_ids', etc.
                # Similar logic to _get_adv_subanswer_and_doc_ids from before, but cleaner
                
                doc_ids = [res['doc_id'] for res in retriever_results]
                poisoned_flags = [res['is_poisoned'] for res in retriever_results]
                
                documents = []
                for res in retriever_results:
                     documents.append(format_input_context({'title': res['title'], 'contents': res['contents']}))
                
                # Reverse to match historical behavior (last doc is most important?)
                # Actually Corag reversed it in _get_subanswer_and_doc_ids. Let's keep consistency.
                documents = documents[::-1]
                poisoned_flags = poisoned_flags[::-1]
                retriever_results = retriever_results[::-1]
                doc_ids = doc_ids[::-1] # Although IDs are fake
